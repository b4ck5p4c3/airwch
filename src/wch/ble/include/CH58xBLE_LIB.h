...
typedef struct tag_ble_config {
  ...
  uint32_t SNVAddr; // SNV flash start address( if NULL,bonding information will
                    // not be saved )
  uint16_t SNVBlock; // SNV flash block size ( default 256 )
  uint8_t SNVNum;    // SNV flash block number ( default 1 )
  uint8_t BufNumber; // Maximum number of sent and received packages cached by
                     // the controller( default 5 ) Must be greater than the
                     // number of connections.
  uint16_t BufMaxLen; // Maximum length (in octets) of the data portion of each
                      // HCI data packet( default 27 ) ATT_MTU =
                      // BufMaxLen-4,Range[23,ATT_MAX_MTU_SIZE]
  uint8_t TxNumEvent; // Maximum number of TX data in a connection event (
                      // default 1 )
  uint8_t RxNumEvent; // Maximum number of RX data in a connection event (
  ...
  uint8_t
      SelRTCClock; // system clock select
                   // bit0-1 00: LSE(32768Hz) 01:LSI(32000Hz) 10:LSI(32768Hz)
                   // bit7:  1: ble timer(HSE)(must disable sleep)
  uint8_t ConnectNumber;    // Connect number,lower two bits are peripheral
                            // number,followed by central number
  ...
  pfnTempSampleCB tsCB;     // Register a program that read the current
                            // temperature,determine whether calibration is need
...
#define MS1_TO_SYSTEM_TIME(x)                                                  \
  ((x) * 1000 / SYSTEM_TIME_MICROSEN) // transform unit in ms to unit in 625us (
                                      // attentional bias )
#define TMOS_TIME_VALID                                                        \
  (30 * 1000 * 1000) // the maximum task time = RTC MAX clock - TMOS_TIME_VALID

/* takes a byte out of a uint32_t : var - uint32_t,  ByteNum - byte to take out
 * (0 - 3) */
...
#define INVALID_MEM_SIZE                                                       \
  0x0B //!< The tokens take up too much space and don't fit into Advertisement
...
#define INVALID_CONNHANDLE                                                     \
  0xFFFF // Invalid connection handle, used for no connection handle
#define LOOPBACK_CONNHANDLE                                                    \
  0xFFFE // Loopback connection handle, used to loopback a message
// Link state flags
...
#define ATT_ERR_INSUFFICIENT_AUTHEN                                            \
  0x05 //!< The attribute requires authentication before it can be read or
       //!< written
#define ATT_ERR_UNSUPPORTED_REQ                                                \
  0x06 //!< Attribute server does not support the request received from the
       //!< client
...
#define ATT_ATTR_HANDLE(info, i)                                               \
  (BUILD_UINT16((info)[ATT_ATTR_HANDLE_IDX((i))],                              \
                (info)[ATT_ATTR_HANDLE_IDX((i)) + 1]))
#define ATT_GRP_END_HANDLE(info, i)                                            \
  (BUILD_UINT16((info)[ATT_GRP_END_HANDLE_IDX((i))],                           \
                (info)[ATT_GRP_END_HANDLE_IDX((i)) + 1]))
/** @defgroup ATT_MSG_EVENT_DEFINES ATT Message Event IDs
 * @{
 */
#define ATT_FLOW_CTRL_VIOLATED_EVENT                                           \
  0x7E //!< Sent when ATT flow control is violated on a connection.  This event
       //!< is sent as an TMOS message defined as attFlowCtrlViolatedEvt_t.
#define ATT_MTU_UPDATED_EVENT                                                  \
  0x7F //!< Sent when MTU is updated for a connection.  This event is sent as an
       //!< TMOS message defined as attMtuUpdatedEvt_t.
...
#define GATT_PROP_WRITE_NO_RSP                                                 \
  0x04 //!< Permits writes of the Characteristic Value without response
#define GATT_PROP_WRITE                                                        \
  0x08 //!< Permits writes of the Characteristic Value with response
#define GATT_PROP_NOTIFY                                                       \
  0x10 //!< Permits notifications of a Characteristic Value without
       //!< acknowledgement
#define GATT_PROP_INDICATE                                                     \
  0x20 //!< Permits indications of a Characteristic Value with acknowledgement
#define GATT_PROP_AUTHEN                                                       \
  0x40 //!< Permits signed writes to the Characteristic Value
#define GATT_PROP_EXTENDED                                                     \
  0x80 //!< Additional characteristic properties are defined in the
...
#define gattPrimaryServiceType(t)                                              \
  (ATT_CompareUUID(primaryServiceUUID, ATT_BT_UUID_SIZE, (t).uuid, (t).len))
#define gattSecondaryServiceType(t)                                            \
  (ATT_CompareUUID(secondaryServiceUUID, ATT_BT_UUID_SIZE, (t).uuid, (t).len))
#define gattCharacterType(t)                                                   \
  (ATT_CompareUUID(characterUUID, ATT_BT_UUID_SIZE, (t).uuid, (t).len))
#define gattIncludeType(t)                                                     \
  (ATT_CompareUUID(includeUUID, ATT_BT_UUID_SIZE, (t).uuid, (t).len))
#define gattServiceType(t)                                                     \
  (gattPrimaryServiceType((t)) || gattSecondaryServiceType((t)))
#define GATT_MAX_NUM_CONN (4)

// GATT Client Characteristic Configuration Bit Fields
#define GATT_CLIENT_CFG_NOTIFY                                                 \
  0x0001 //!< The Characteristic Value shall be notified
#define GATT_CLIENT_CFG_INDICATE                                               \
...
// The handle of the first included service (i = 1) is the value of the second
// attribute
...
#define GAP_DEVICE_INIT_DONE_EVENT                                             \
  0x00 //!< Sent when the Device Initialization is complete.  This event is sent
       //!< as an tmos message defined as gapDeviceInitDoneEvent_t.
#define GAP_DEVICE_DISCOVERY_EVENT                                             \
  0x01 //!< Sent when the Device Discovery Process is complete. This event is
       //!< sent as an tmos message defined as gapDevDiscEvent_t.
#define GAP_ADV_DATA_UPDATE_DONE_EVENT                                         \
  0x02 //!< Sent when the Advertising Data or SCAN_RSP Data has been updated.
       //!< This event is sent as an tmos message defined as
       //!< gapAdvDataUpdateEvent_t.
#define GAP_MAKE_DISCOVERABLE_DONE_EVENT                                       \
  0x03 //!< Sent when the Make Discoverable Request is complete. This event is
       //!< sent as an tmos message defined as gapMakeDiscoverableRspEvent_t.
#define GAP_END_DISCOVERABLE_DONE_EVENT                                        \
  0x04 //!< Sent when the Advertising has ended. This event is sent as an tmos
       //!< message defined as gapEndDiscoverableRspEvent_t.
#define GAP_LINK_ESTABLISHED_EVENT                                             \
  0x05 //!< Sent when the Establish Link Request is complete. This event is sent
       //!< as an tmos message defined as gapEstLinkReqEvent_t.
#define GAP_LINK_TERMINATED_EVENT                                              \
  0x06 //!< Sent when a connection was terminated. This event is sent as an tmos
       //!< message defined as gapTerminateLinkEvent_t.
#define GAP_LINK_PARAM_UPDATE_EVENT                                            \
  0x07 //!< Sent when an Update Parameters Event is received. This event is sent
       //!< as an tmos message defined as gapLinkUpdateEvent_t.
#define GAP_RANDOM_ADDR_CHANGED_EVENT                                          \
  0x08 //!< Sent when a random address was changed. This event is sent as an
       //!< tmos message defined as gapRandomAddrEvent_t.
#define GAP_SIGNATURE_UPDATED_EVENT                                            \
  0x09 //!< Sent when the device's signature counter is updated. This event is
       //!< sent as an tmos message defined as gapSignUpdateEvent_t.
#define GAP_AUTHENTICATION_COMPLETE_EVENT                                      \
  0x0A //!< Sent when the Authentication (pairing) process is complete. This
       //!< event is sent as an tmos message defined as gapAuthCompleteEvent_t.
#define GAP_PASSKEY_NEEDED_EVENT                                               \
  0x0B //!< Sent when a Passkey is needed.  This is part of the pairing process.
       //!< This event is sent as an tmos message defined as
       //!< gapPasskeyNeededEvent_t.
#define GAP_SLAVE_REQUESTED_SECURITY_EVENT                                     \
  0x0C //!< Sent when a Slave Security Request is received. This event is sent
       //!< as an tmos message defined as gapSlaveSecurityReqEvent_t.
#define GAP_DEVICE_INFO_EVENT                                                  \
  0x0D //!< Sent during the Device Discovery Process when a device is
       //!< discovered. This event is sent as an tmos message defined as
       //!< gapDeviceInfoEvent_t.
#define GAP_BOND_COMPLETE_EVENT                                                \
  0x0E //!< Sent when the bonding process is complete. This event is sent as an
       //!< tmos message defined as gapBondCompleteEvent_t.
#define GAP_PAIRING_REQ_EVENT                                                  \
  0x0F //!< Sent when an unexpected Pairing Request is received. This event is
       //!< sent as an tmos message defined as gapPairingReqEvent_t.
#define GAP_DIRECT_DEVICE_INFO_EVENT                                           \
  0x10 //!< Sent when a direct Advertising Data is received. This event is sent
       //!< as an tmos message defined as gapDirectDeviceInfoEvent_t.
#define GAP_PHY_UPDATE_EVENT                                                   \
  0x11 //!< Sent when a PHY Update Event is received. This event is sent as an
       //!< tmos message defined as gapLinkUpdateEvent_t.
#define GAP_EXT_ADV_DEVICE_INFO_EVENT                                          \
  0x12 //!< Sent when a Extended Advertising Data is received. This event is
       //!< sent as an tmos message defined as gapExtAdvDeviceInfoEvent_t.
#define GAP_MAKE_PERIODIC_ADV_DONE_EVENT                                       \
  0x13 //!< Sent when the Set Periodic Advertising enable is complete. This
       //!< event is sent as an tmos message defined as
       //!< gapMakePeriodicRspEvent_t.
#define GAP_END_PERIODIC_ADV_DONE_EVENT                                        \
  0x14 //!< Sent when the Set Periodic Advertising disable is complete. This
       //!< event is sent as an tmos message defined as
       //!< gapEndPeriodicRspEvent_t.
#define GAP_SYNC_ESTABLISHED_EVENT                                             \
  0x15 //!< Sent when a Periodic Advertising Sync Establish is complete. This
       //!< event is sent as an tmos message defined as
       //!< gapSyncEstablishedEvent_t.
#define GAP_PERIODIC_ADV_DEVICE_INFO_EVENT                                     \
  0x16 //!< Sent when a Periodic Advertising Data is received. This event is
       //!< sent as an tmos message defined as gapPeriodicAdvDeviceInfoEvent_t.
#define GAP_SYNC_LOST_EVENT                                                    \
  0x17 //!< Sent when a Periodic Advertising Sync was lost. This event is sent
       //!< as an tmos message defined as gapSyncLostEvent_t.
#define GAP_SCAN_REQUEST_EVENT                                                 \
  0x19 //!< Sent when a SCAN_REQ PDU or an AUX_SCAN_REQ PDU has been received by
       //!< the advertiser. This event is sent as an tmos message defined as
       //!< gapScanReqReseiveEvent_t.

// GAP_PROFILE_ROLE_DEFINES GAP Profile Roles
#define GAP_PROFILE_BROADCASTER                                                \
  0x01 //!< A device that sends advertising events only.
#define GAP_PROFILE_OBSERVER                                                   \
  0x02 //!< A device that receives advertising events only.
#define GAP_PROFILE_PERIPHERAL                                                 \
  0x04 //!< A device that accepts the establishment of an LE physical link using
       //!< the connection establishment procedure
#define GAP_PROFILE_CENTRAL                                                    \
  0x08 //!< A device that supports the Central role initiates the establishment
...
// option defined
#define LISTEN_PERIODIC_ADVERTISING_MODE                                       \
  (1 << 0) //!< used to determine whether the Periodic Advertiser List is used
#define REPORTING_INITIALLY_DISABLED                                           \
  (1 << 1) //!< 0: Reporting initially enabled 1: Reporting initially disabled
#define DUPLICATE_FILTERING_INITIALLY_ENABLED                                  \
  (1 << 2) //!< 0: Duplicate filtering initially disabled 1: Duplicate filtering
           //!< initially enabled
...
#define GAP_CONNHANDLE_ALL                                                     \
  0xFFFF //!< terminates all links for the matching task ID.
...
// GAP GATT Server Parameters used with GGS Get/Set Parameter and Application's
// Callback functions
...
#define TGAP_GEN_DISC_ADV_MIN                                                  \
  0 //!< Minimum time to remain advertising, when in Discoverable mode.Default
    //!< 0-turns off the timeout. (n * 0.625 mSec).
#define TGAP_LIM_ADV_TIMEOUT                                                   \
  1 //!< Maximum time to remain advertising, when in Limited Discoverable
    //!< mode.Default 180 seconds. (n * 1 seconds)
#define TGAP_DISC_SCAN                                                         \
  2 //!< Minimum time to perform scanning,Setting this parameter to 0 turns off
    //!< the timeout.Default 10.24seconds. (n * 0.625 mSec)

// when in General Discovery process
#define TGAP_DISC_ADV_INT_MIN                                                  \
  3 //!< Minimum advertising interval.Default 160. (n * 0.625 mSec)
#define TGAP_DISC_ADV_INT_MAX                                                  \
  4 //!< Maximum advertising interval.Default 160. (n * 0.625 mSec)
#define TGAP_DISC_SCAN_INT                                                     \
  5 //!< Scan interval used during Link Layer Scanning state.Default 16. (n *
    //!< 0.625 mSec)
#define TGAP_DISC_SCAN_WIND                                                    \
  6 //!< Scan window used during Link Layer Scanning state.Default 16. (n *
    //!< 0.625 mSec)

// when in Connection Establishment process(1M PHY)
#define TGAP_CONN_EST_INT_MIN                                                  \
  7 //!< Minimum Link Layer connection interval.Default 80. (n * 1.25 mSec)
#define TGAP_CONN_EST_INT_MAX                                                  \
  8 //!< Maximum Link Layer connection interval.Default 80. (n * 1.25 mSec)
#define TGAP_CONN_EST_SCAN_INT                                                 \
  9 //!< Scan interval used during Link Layer Initiating state.Default 16. (n *
    //!< 0.625 mSec)
#define TGAP_CONN_EST_SCAN_WIND                                                \
  10 //!< Scan window used during Link Layer Initiating state.Default 16. (n *
     //!< 0.625 mSec)
#define TGAP_CONN_EST_HIGH_SCAN_INT                                            \
  11 //!< Scan interval used during Link Layer Initiating state, high duty scan
     //!< cycle scan parameters (n * 0.625 mSec)
#define TGAP_CONN_EST_HIGH_SCAN_WIND                                           \
  12 //!< Scan window used during Link Layer Initiating state, high duty scan
     //!< cycle scan parameters (n * 0.625 mSec)
#define TGAP_CONN_EST_SUPERV_TIMEOUT                                           \
  13 //!< Link Layer connection supervision timeout.Default 2000. (n * 10 mSec)
#define TGAP_CONN_EST_LATENCY                                                  \
  14 //!< Link Layer connection slave latency.Default 0. (in number of
     //!< connection events)
#define TGAP_CONN_EST_MIN_CE_LEN                                               \
  15 //!< Local informational parameter about minimum length of connection
     //!< needed.Default 0. (n * 0.625 mSec)
#define TGAP_CONN_EST_MAX_CE_LEN                                               \
  16 //!< Local informational parameter about maximum length of connection
     //!< needed.Default 0. (n * 0.625 mSec)

// Proprietary
#define TGAP_PRIVATE_ADDR_INT                                                  \
  17 //!< Minimum Time Interval between private (resolvable) address
     //!< changes.Default 900. (n * 1 seconds)
#define TGAP_SM_TIMEOUT                                                        \
  18 //!< SM Message Timeout (milliseconds). Default 30 seconds.
...
#define TGAP_FILTER_ADV_REPORTS                                                \
  21 //!< Filter duplicate advertising reports. Default TRUE.
#define TGAP_SCAN_RSSI_MIN                                                     \
  22 //!< Minimum RSSI required for scan advertising to be reported to the app.
     //!< Default -127.
#define TGAP_REJECT_CONN_PARAMS                                                \
  23 //!< Whether or not to reject Connection Parameter Update Request received
     //!< on Central device. Default FALSE.
#define TGAP_AUTH_TASK_ID                                                      \
  24 //!< Task ID override for Task Authentication control (for stack internal
     //!< use only)

// v5.x
#define TGAP_ADV_TX_POWER                                                      \
  25 //!< Indicates the maximum power level Range: -127 �� N �� +126 Units:
     //!< dBm.Default 127(Host has no preference).
#define TGAP_ADV_PRIMARY_PHY                                                   \
  26 //!< Indicates the PHY on which the advertising packets are transmitted on
     //!< the primary advertising channel.LE 1M/LE Coded.Default
     //!< GAP_PHY_VAL_LE_1M.
#define TGAP_ADV_SECONDARY_PHY                                                 \
  27 //!< LE 1M/LE 2M/LE Coded. Default GAP_PHY_VAL_LE_1M.
#define TGAP_ADV_SECONDARY_MAX_SKIP                                            \
  28 //!< Maximum advertising events the Controller can skip before sending the
     //!< AUX_ADV_IND packets on the secondary advertising channel. Default 0.
#define TGAP_ADV_ADVERTISING_SID                                               \
  29 //!< Value of the Advertising SID subfield in the ADI field of the PDU
     //!< Range:0-15. Default 0.
#define TGAP_ADV_SCAN_REQ_NOTIFY                                               \
  30 //!< Scan request notifications enabled.Default 0-disabled.
#define TGAP_ADV_ADVERTISING_DURATION                                          \
  31 //!< Advertising duration Range: 0x0001 - 0xFFFF Time = N * 10ms. Default
     //!< 0-No advertising duration.
#define TGAP_ADV_MAX_EVENTS                                                    \
  32 //!< indicates the maximum number of extended advertising events.Range:
     //!< 0x00 - 0xFF. Default 0(No maximum number of advertising events).

// when in General Discovery process
#define TGAP_DISC_SCAN_PHY 33 //!< LE 1M/LE Coded. Default GAP_PHY_BIT_LE_1M.
#define TGAP_DISC_SCAN_CODED_INT                                               \
  34 //!< Scan interval used during Link Layer coded Scanning state, when in
     //!< General Discovery process (n * 0.625 mSec)
#define TGAP_DISC_SCAN_CODED_WIND                                              \
  35 //!< Scan window used during Link Layer coded Scanning state, when in
     //!< General Discovery process (n * 0.625 mSec)
#define TGAP_DISC_SCAN_DURATION                                                \
  36 //!< Scan duration Range: 0x0001 - 0xFFFF Time = N * 10 ms. Default 0-Scan
     //!< continuously until explicitly disable.
...
#define TGAP_CONN_EST_2M_INT_MIN                                               \
  39 //!< Minimum Link Layer connection interval.Default 80. (n * 1.25 mSec)
#define TGAP_CONN_EST_2M_INT_MAX                                               \
  40 //!< Maximum Link Layer connection interval.Default 80. (n * 1.25 mSec)
#define TGAP_CONN_EST_2M_SUPERV_TIMEOUT                                        \
  41 //!< Link Layer connection supervision timeout.Default 2000. (n * 10 mSec)
#define TGAP_CONN_EST_2M_LATENCY                                               \
  42 //!< Link Layer connection slave latency.Default 0. (in number of
     //!< connection events)
#define TGAP_CONN_EST_2M_MIN_CE_LEN                                            \
  43 //!< Local informational parameter about minimum length of connection
     //!< needed.Default 0. (n * 0.625 mSec)
#define TGAP_CONN_EST_2M_MAX_CE_LEN                                            \
  44 //!< Local informational parameter about maximum length of connection
     //!< needed.Default 0. (n * 0.625 mSec)

// when in Connection Establishment process(Coded PHY)
#define TGAP_CONN_EST_CODED_INT_MIN                                            \
  45 //!< Minimum Link Layer connection interval.Default 80. (n * 1.25 mSec)
#define TGAP_CONN_EST_CODED_INT_MAX                                            \
  46 //!< Maximum Link Layer connection interval.Default 80. (n * 1.25 mSec)
#define TGAP_CONN_EST_CODED_SCAN_INT                                           \
  47 //!< Scan interval used during Link Layer Initiating state.Default 16. (n *
     //!< 0.625 mSec)
#define TGAP_CONN_EST_CODED_SCAN_WIND                                          \
  48 //!< Scan window used during Link Layer Initiating state.Default 16. (n *
     //!< 0.625 mSec)
#define TGAP_CONN_EST_CODED_HIGH_SCAN_INT                                      \
  49 //!< Scan interval used during Link Layer Initiating state, high duty scan
     //!< cycle scan parameters (n * 0.625 mSec)
#define TGAP_CONN_EST_CODED_HIGH_SCAN_WIND                                     \
  50 //!< Scan window used during Link Layer Initiating state, high duty scan
     //!< cycle scan parameters (n * 0.625 mSec)
#define TGAP_CONN_EST_CODED_SUPERV_TIMEOUT                                     \
  51 //!< Link Layer connection supervision timeout.Default 2000. (n * 10 mSec)
#define TGAP_CONN_EST_CODED_LATENCY                                            \
  52 //!< Link Layer connection slave latency.Default 0. (in number of
     //!< connection events)
#define TGAP_CONN_EST_CODED_MIN_CE_LEN                                         \
  53 //!< Local informational parameter about minimum length of connection
     //!< needed.Default 0. (n * 0.625 mSec)
#define TGAP_CONN_EST_CODED_MAX_CE_LEN                                         \
  54 //!< Local informational parameter about maximum length of connection
     //!< needed.Default 0. (n * 0.625 mSec)

// periodic advertising
#define TGAP_PERIODIC_ADV_INT_MIN                                              \
  55 //!< Minimum periodic advertising interval.Range: 0x0006 to 0xFFFF.Default
     //!< 160. (n * 1.25 mSec)
#define TGAP_PERIODIC_ADV_INT_MAX                                              \
  56 //!< Maximum periodic advertising interval.Range: 0x0006 to 0xFFFF.Default
     //!< 160. (n * 1.25 mSec)
#define TGAP_PERIODIC_ADV_PROPERTIES                                           \
  57 //!< Include TxPower in the periodic advertising PDU.

#define TGAP_SCAN_MAX_LENGTH                                                   \
  58 //!< Extended scan maximum data length.Default 460
#define TGAP_AFH_CHANNEL_MDOE                                                  \
  59 //!< whether t he Controller's channel assessment scheme is enabled or
...
#define ADDRTYPE_PRIVATE_NONRESOLVE                                            \
...
#define GAP_ADTYPE_ADV_HDC_DIRECT_IND                                          \
  0x01 //!< Connectable high duty cycle directed event type
#define GAP_ADTYPE_ADV_SCAN_IND 0x02 //!< Scannable undirected event type
#define GAP_ADTYPE_ADV_NONCONN_IND                                             \
  0x03 //!< Non-Connectable undirected event type
#define GAP_ADTYPE_ADV_LDC_DIRECT_IND                                          \
  0x04 //!< Connectable low duty cycle directed event type
// v5.x
#define GAP_ADTYPE_EXT_CONN_DIRECT                                             \
  0x05 //!< extend Connectable directed event type
#define GAP_ADTYPE_EXT_SCAN_UNDIRECT                                           \
  0x06 //!< extend Scannable undirected event type
#define GAP_ADTYPE_EXT_NONCONN_NONSCAN_UNDIRECT                                \
  0x07 //!< extend Non-Connectable and Non-Scannable undirected event type
#define GAP_ADTYPE_EXT_CONN_UNDIRECT                                           \
  0x08 //!< extend Connectable undirected event type
#define GAP_ADTYPE_EXT_SCAN_DIRECT                                             \
  0x09 //!< extend Scannable directed event type
#define GAP_ADTYPE_EXT_NONCONN_NONSCAN_DIRECT                                  \
  0x0A //!< extend Non-Connectable and Non-Scannable directed event type
...
#define GAP_PHY_BIT_ALL                                                        \
  (GAP_PHY_BIT_LE_1M | GAP_PHY_BIT_LE_2M | GAP_PHY_BIT_LE_CODED)
...
// bit0 to 4 ADVERTISEMENT_TYPE:defined for gapExtAdvDeviceInfoEvent_t
...
// bit5 to 6 Data status:defined for gapExtAdvDeviceInfoEvent_t Advertisement
// data type
...
// GAP_EXTEND_ADVERTISEMENT_REPORT_TYPE_DEFINES GAP Extend Advertising Report
// Event Types
...
#define GAP_FILTER_POLICY_ALL                                                  \
  0x00 //!< Allow Scan Request from Any, Allow Connect Request from Any
       //!< (default).
#define GAP_FILTER_POLICY_WHITE_SCAN                                           \
  0x01 //!< Allow Scan Request from White List Only, Allow Connect from Any
#define GAP_FILTER_POLICY_WHITE_CON                                            \
  0x02 //!< Allow Scan Request from Any, Connect from White List Only
#define GAP_FILTER_POLICY_WHITE                                                \
  0x03 //!< Allow Scan Request and Connect from White List Only

//! Maximum Pairing Passcode/Passkey value.  Range of a passkey can be 0 -
//! 999,999.
#define GAP_PASSCODE_MAX 999999

/** Sign Counter Initialized - Sign counter hasn't been used yet.  Used when
 * setting up a connection's signing information.
 */
...
#define GAP_ADVCHAN_ALL                                                        \
  (GAP_ADVCHAN_37 | GAP_ADVCHAN_38 |                                           \
...
#define GAP_ADTYPE_INDOOR_POSITION                                             \
  0x25 //!< Indoor Positioning Service v1.0 or later
#define GAP_ADTYPE_TRAN_DISCOVERY_DATA                                         \
  0x26 //!< Transport Discovery Service v1.0 or later
...
#define GAP_ADTYPE_RSL_SET_IDENT                                               \
  0x2E //!< Resolvable Set Identifier.Coordinated Set Identification Profile 1.0
...
#define GAP_ADTYPE_FLAGS_LIMITED                                               \
  0x01 //!< Discovery Mode: LE Limited Discoverable Mode
#define GAP_ADTYPE_FLAGS_GENERAL                                               \
  0x02 //!< Discovery Mode: LE General Discoverable Mode
#define GAP_ADTYPE_FLAGS_BREDR_NOT_SUPPORTED                                   \
...
#define GAPROLE_PROFILEROLE                                                    \
  0x300 //!< Reading this parameter will return GAP Role type. Read Only. Size
        //!< is uint8_t.
#define GAPROLE_IRK                                                            \
  0x301 //!< Identity Resolving Key. Read/Write. Size is uint8_t[KEYLEN].
        //!< Default is all 0, which means that the IRK will be randomly
        //!< generated.
#define GAPROLE_SRK                                                            \
  0x302 //!< Signature Resolving Key. Read/Write. Size is uint8_t[KEYLEN].
        //!< Default is all 0, which means that the SRK will be randomly
        //!< generated.
#define GAPROLE_SIGNCOUNTER                                                    \
  0x303 //!< Sign Counter. Read/Write. Size is uint32_t. Default is 0.
#define GAPROLE_BD_ADDR                                                        \
  0x304 //!< Device's Address. Read Only. Size is uint8_t[B_ADDR_LEN]. This item
        //!< is read from the controller.
#define GAPROLE_ADVERT_ENABLED                                                 \
  0x305 //!< Enable/Disable Advertising. Read/Write. Size is uint8_t. Default is
        //!< TRUE=Enabled.
#define GAPROLE_ADVERT_DATA                                                    \
  0x306 //!< Advertisement Data. Read/Write. Max size is B_MAX_ADV_EXT_LEN.
        //!< Default to all 0.
#define GAPROLE_SCAN_RSP_DATA                                                  \
  0x307 //!< Scan Response Data. Read/Write. Max size is B_MAX_ADV_EXT_LEN.
        //!< Defaults to all 0.
#define GAPROLE_ADV_EVENT_TYPE                                                 \
  0x308 //!< Advertisement Type. Read/Write. Size is uint8_t.  Default is
        //!< GAP_ADTYPE_ADV_IND.
#define GAPROLE_ADV_DIRECT_TYPE                                                \
  0x309 //!< Direct Advertisement Address Type. Read/Write. Size is uint8_t.
        //!< Default is ADDRTYPE_PUBLIC.
#define GAPROLE_ADV_DIRECT_ADDR                                                \
  0x30A //!< Direct Advertisement Address. Read/Write. Size is
        //!< uint8_t[B_ADDR_LEN]. Default is NULL.
#define GAPROLE_ADV_CHANNEL_MAP                                                \
  0x30B //!< Which channels to advertise on. Read/Write Size is uint8_t. Default
        //!< is GAP_ADVCHAN_ALL
#define GAPROLE_ADV_FILTER_POLICY                                              \
  0x30C //!< Filter Policy. Ignored when directed advertising is used.
        //!< Read/Write. Size is uint8_t. Default is GAP_FILTER_POLICY_ALL.
#define GAPROLE_STATE                                                          \
  0x30D //!< Reading this parameter will return GAP Peripheral Role State. Read
        //!< Only. Size is uint8_t.
#define GAPROLE_MAX_SCAN_RES                                                   \
  0x30E //!< Maximum number of discover scan results to receive. Default is 0 =
        //!< unlimited.
#define GAPROLE_MIN_CONN_INTERVAL                                              \
  0x311 //!< Minimum Connection Interval to allow (n * 1.25ms).  Range: 7.5 msec
        //!< to 4 seconds (0x0006 to 0x0C80). Read/Write. Size is uint16_t.
        //!< Default is 7.5 milliseconds (0x0006).
#define GAPROLE_MAX_CONN_INTERVAL                                              \
  0x312 //!< Maximum Connection Interval to allow (n * 1.25ms).  Range: 7.5 msec
        //!< to 4 seconds (0x0006 to 0x0C80). Read/Write. Size is uint16_t.
        //!< Default is 4 seconds (0x0C80).
// v5.x
#define GAPROLE_PHY_TX_SUPPORTED                                               \
  0x313 //!< The transmitter PHYs that the Host prefers the Controller to
        //!< use.Default is GAP_PHY_BIT_ALL
#define GAPROLE_PHY_RX_SUPPORTED                                               \
  0x314 //!< The receiver PHYs that the Host prefers the Controller to
        //!< use.Default is GAP_PHY_BIT_ALL
#define GAPROLE_PERIODIC_ADVERT_DATA                                           \
  0x315 //!< Periodic advertisement Data. Read/Write. Max size is
        //!< B_MAX_ADV_PERIODIC_LEN. Default to all 0.
#define GAPROLE_PERIODIC_ADVERT_ENABLED                                        \
  0x316 //!< bit0:Enable/Disable Periodic Advertising. Read/Write. Size is
        //!< uint8_t. Default is FALSE=Disable. bit1:Include the ADI field in
        //!< AUX_SYNC_IND PDUs

/************************************GAPBOND***********************************/
// GAPBOND_PROFILE_PARAMETERS GAP Bond Manager Parameters
#define GAPBOND_PERI_PAIRING_MODE                                              \
  0x400 //!< Pairing Mode: @ref  GAPBOND_PAIRING_MODE_DEFINES. Read/Write. Size
        //!< is uint8_t. Default is GAPBOND_PAIRING_MODE_WAIT_FOR_REQ.
#define GAPBOND_PERI_MITM_PROTECTION                                           \
  0x401 //!< Man-In-The-Middle (MITM) basically turns on Passkey protection in
        //!< the pairing algorithm. Read/Write. Size is uint8_t. Default is
        //!< 0(disabled).
#define GAPBOND_PERI_IO_CAPABILITIES                                           \
  0x402 //!< I/O capabilities.  Read/Write. Size is uint8_t. Default is
        //!< GAPBOND_IO_CAP_DISPLAY_ONLY @ref GAPBOND_IO_CAP_DEFINES.
#define GAPBOND_PERI_OOB_ENABLED                                               \
  0x403 //!< OOB data available for pairing algorithm. Read/Write. Size is
        //!< uint8_t. Default is 0(disabled).
#define GAPBOND_PERI_OOB_DATA                                                  \
  0x404 //!< OOB Data. Read/Write. size uint8_t[16]. Default is all 0's.
#define GAPBOND_PERI_BONDING_ENABLED                                           \
  0x405 //!< Request Bonding during the pairing process if enabled.  Read/Write.
        //!< Size is uint8_t. Default is 0(disabled).
#define GAPBOND_PERI_KEY_DIST_LIST                                             \
  0x406 //!< The key distribution list for bonding.  size is uint8_t.  @ref
        //!< GAPBOND_KEY_DIST_DEFINES. Default is 7.
#define GAPBOND_PERI_DEFAULT_PASSCODE                                          \
  0x407 //!< The default passcode for MITM protection. size is uint32_t. Range
        //!< is 0 - 999,999. Default is 0.
#define GAPBOND_CENT_PAIRING_MODE                                              \
  0x408 //!< Pairing Mode: @ref  GAPBOND_PAIRING_MODE_DEFINES. Read/Write. Size
        //!< is uint8_t. Default is GAPBOND_PAIRING_MODE_WAIT_FOR_REQ.
#define GAPBOND_CENT_MITM_PROTECTION                                           \
  0x409 //!< Man-In-The-Middle (MITM) basically turns on Passkey protection in
        //!< the pairing algorithm. Read/Write. Size is uint8_t. Default is
        //!< 0(disabled).
#define GAPBOND_CENT_IO_CAPABILITIES                                           \
  0x40A //!< I/O capabilities.  Read/Write. Size is uint8_t. Default is
        //!< GAPBOND_IO_CAP_DISPLAY_ONLY @ref GAPBOND_IO_CAP_DEFINES.
#define GAPBOND_CENT_OOB_ENABLED                                               \
  0x40B //!< OOB data available for pairing algorithm. Read/Write. Size is
        //!< uint8_t. Default is 0(disabled).
#define GAPBOND_CENT_OOB_DATA                                                  \
  0x40C //!< OOB Data. Read/Write. size uint8_t[16]. Default is all 0's.
#define GAPBOND_CENT_BONDING_ENABLED                                           \
  0x40D //!< Request Bonding during the pairing process if enabled.  Read/Write.
        //!< Size is uint8_t. Default is 0(disabled).
#define GAPBOND_CENT_KEY_DIST_LIST                                             \
  0x40E //!< The key distribution list for bonding.  size is uint8_t.  @ref
        //!< GAPBOND_KEY_DIST_DEFINES. Default is 7.
#define GAPBOND_CENT_DEFAULT_PASSCODE                                          \
  0x40F //!< The default passcode for MITM protection. size is uint32_t. Range
        //!< is 0 - 999,999. Default is 0.
#define GAPBOND_ERASE_ALLBONDS                                                 \
  0x410 //!< Erase all of the bonded devices. Write Only. No Size.
#define GAPBOND_AUTO_FAIL_PAIRING                                              \
  0x411 //!< TEST MODE (DO NOT USE) to automatically send a Pairing Fail when a
        //!< Pairing Request is received. Read/Write. size is uint8_t. Default
        //!< is 0 (disabled).
#define GAPBOND_AUTO_FAIL_REASON                                               \
  0x412 //!< TEST MODE (DO NOT USE) Pairing Fail reason when auto failing.
        //!< Read/Write. size is uint8_t. Default is 0x05
        //!< (SMP_PAIRING_FAILED_NOT_SUPPORTED).
#define GAPBOND_KEYSIZE                                                        \
  0x413 //!< Key Size used in pairing. Read/Write. size is uint8_t. Default
        //!< is 16.
#define GAPBOND_AUTO_SYNC_WL                                                   \
  0x414 //!< Clears the White List adds to it each unique address stored by
        //!< bonds in NV. Read/Write. Size is uint8_t. Default is FALSE.
#define GAPBOND_BOND_COUNT                                                     \
  0x415 //!< Gets the total number of bonds stored in NV. Read Only. Size is
        //!< uint8_t. Default is 0 (no bonds).
#define GAPBOND_BOND_FAIL_ACTION                                               \
  0x416 //!< Possible actions Central may take upon an unsuccessful bonding.
        //!< Write Only. Size is uint8_t. Default is 0x02 (Terminate link upon
        //!< unsuccessful bonding).
#define GAPBOND_ERASE_SINGLEBOND                                               \
  0x417 //!< Erase a single bonded device. Write only. Must provide address type
        //!< followed by device address.
#define GAPBOND_BOND_AUTO                                                      \
  0x418 //!< Auto save bonds into FLASH. Write Only. size is uint8_t. Default is
        //!< 1(enabled).
#define GAPBOND_BOND_UPDATE                                                    \
  0x419 //!< Save current bonds into FLASH. Write Only. No Size.
#define GAPBOND_DISABLE_SINGLEBOND                                             \
  0x41A //!< Disable a single bonded device. Write only. Must provide address
        //!< type followed by device address.
#define GAPBOND_ENABLE_SINGLEBOND                                              \
  0x41B //!< Ensable a single bonded device. Write only. Must provide address
        //!< type followed by device address.
#define GAPBOND_DISABLE_ALLBONDS                                               \
  0x41C //!< Disable all of the bonded devices. Write Only. No Size.
#define GAPBOND_ENABLE_ALLBONDS                                                \
  0x41D //!< Ensable all of the bonded devices. Write Only. No Size.
#define GAPBOND_ERASE_AUTO                                                     \
  0x41E //!< Auto erase all of the bonded devices when the maximum number is
        //!< reached.Size is uint8_t. Default is 1(enabled).
#define GAPBOND_AUTO_SYNC_RL                                                   \
  0x41F //!< Clears the Resolving List adds to it each unique address stored by
        //!< bonds in NV. Read/Write. Size is uint8_t. Default is FALSE.
#define GAPBOND_SET_ENC_PARAMS                                                 \
  0x420 //!< Set bonding parameters.size is bondEncParams_t.
...
#define GAPBOND_PAIRING_MODE_WAIT_FOR_REQ                                      \
  0x01 //!< Wait for a pairing request or slave security request
#define GAPBOND_PAIRING_MODE_INITIATE                                          \
...
#define SMP_PAIRING_FAILED_PASSKEY_ENTRY_FAILED                                \
  0x01 //!< The user input of the passkey failed, for example, the user
...
#define SMP_PAIRING_FAILED_AUTH_REQ                                            \
  0x03 //!< The pairing procedure can't be performed as authentication
       //!< requirements can't be met due to IO capabilities of one or both
       //!< devices
#define SMP_PAIRING_FAILED_CONFIRM_VALUE                                       \
  0x04 //!< The confirm value doesn't match the calculated compare value
#define SMP_PAIRING_FAILED_NOT_SUPPORTED                                       \
  0x05 //!< Pairing isn't supported by the device
#define SMP_PAIRING_FAILED_ENC_KEY_SIZE                                        \
  0x06 //!< The resultant encryption key size is insufficient for the security
       //!< requirements of this device.
#define SMP_PAIRING_FAILED_CMD_NOT_SUPPORTED                                   \
  0x07 //!< The SMP command received is not supported on this device.
#define SMP_PAIRING_FAILED_UNSPECIFIED                                         \
  0x08 //!< Pairing failed due to an unspecified reason
#define SMP_PAIRING_FAILED_REPEATED_ATTEMPTS                                   \
  0x09 //!< Pairing or authentication procedure is disallowed because too little
       //!< time has elapsed since the last pairing request or security request.

// GAPBOND_BONDING_FAILURE_DEFINES Bonding Failure Actions
#define GAPBOND_FAIL_NO_ACTION                                                 \
  0x00 //!< Take no action upon unsuccessful bonding
#define GAPBOND_FAIL_INITIATE_PAIRING                                          \
  0x01 //!< Initiate pairing upon unsuccessful bonding
#define GAPBOND_FAIL_TERMINATE_LINK                                            \
  0x02 //!< Terminate link upon unsuccessful bonding
#define GAPBOND_FAIL_TERMINATE_ERASE_BONDS                                     \
  0x03 //!< Terminate link and erase all existing bonds on device upon
       //!< unsuccessful bonding
...
#define GAP_BOND_LOCAL_LTK_OFFSET                                              \
  1 //!< NV ID for the bonding record's local LTK information
#define GAP_BOND_DEV_LTK_OFFSET                                                \
  2 //!< NV ID for the bonding records' device LTK information
#define GAP_BOND_DEV_IRK_OFFSET 3 //!< NV ID for the bonding records' device IRK
#define GAP_BOND_DEV_CSRK_OFFSET                                               \
  4 //!< NV ID for the bonding records' device CSRK
#define GAP_BOND_DEV_SIGN_COUNTER_OFFSET                                       \
  5 //!< NV ID for the bonding records' device Sign Counter
#define GAP_BOND_REC_IDS 6

// Macros to calculate the index/offset in to NV space
#define calcNvID(Idx, offset)                                                  \
  (((((Idx) * GAP_BOND_REC_IDS) + (offset))) + BLE_NVID_GAP_BOND_START)
...
#define devSignCounterNvID(bondIdx)                                            \
  (calcNvID((bondIdx), GAP_BOND_DEV_SIGN_COUNTER_OFFSET))
...
typedef struct {
  ...
  uint16_t stateFlags; // State flags: SM_AUTH_STATE_AUTHENTICATED &
...
typedef struct {
  ...
  uint8_t *pInfo;   //!< Information data whose format is determined by format
                    //!< field (4 to ATT_MTU_SIZE-2)
} attFindInfoRsp_t;
...
typedef struct {
  ...
  uint8_t *pHandlesInfo; //!< List of 1 or more handles information (4 to
                         //!< ATT_MTU_SIZE-1)
...
typedef struct {
  ...
  uint8_t *pDataList; //!< List of 1 or more attribute handle-value pairs (2 to
                      //!< ATT_MTU_SIZE-2)
} attReadByTypeRsp_t;
...
typedef struct {
  uint16_t len;    //!< Length of value
  uint8_t *pValue; //!< Value of the attribute with the handle given (0 to
                   //!< ATT_MTU_SIZE-1)
} attReadRsp_t;
...
typedef struct {
  uint16_t len;    //!< Length of value
  uint8_t *pValue; //!< Part of the value of the attribute with the handle given
                   //!< (0 to ATT_MTU_SIZE-1)
} attReadBlobRsp_t;
 ...
 */
typedef struct {
  uint8_t *pHandles;   //!< Set of two or more attribute handles (4 to
                       //!< ATT_MTU_SIZE-1) - must be first field
 ...
 */
typedef struct {
  uint16_t numGrps; //!< Number of attribute handle, end group handle and value
                    //!< sets found
  uint16_t
      len; //!< Length of each attribute handle, end group handle and value set
  uint8_t *pDataList; //!< List of 1 or more attribute handle, end group handle
                      //!< and value (4 to ATT_MTU_SIZE-2)
...
typedef struct {
  ...
  uint8_t sig; //!< Authentication Signature status (not included (0), valid
               //!< (1), invalid (2))
...
typedef struct {
  ...
  uint16_t len;    //!< Length of value
  uint8_t *pValue; //!< Part of the value of the attribute to be written (0 to
                   //!< ATT_MTU_SIZE-5) - must be allocated
 ...
 */
typedef struct {
  uint16_t handle; //!< Handle of the attribute that has been changed (must be
                   //!< first field)
  uint16_t len;    //!< Length of value
 ...
 * or Indication-Confirmation protocol flow control is violated for a
 * connection. All subsequent ATT Requests and Indications received by the local
 * ATT Server and Client respectively will be dropped.
 *
 ...
 *
 * This message is to inform the app (that has registered with GAP by calling
 * GAP_RegisterForMsgs()) about the new ATT MTU size negotiated for a
 * connection.
 */
typedef struct {
 ...
 */
typedef struct {
  uint8_t discCharsByUUID; //!< Whether this is a GATT Discover Characteristics
                           //!< by UUID sub-procedure
 ...
 */
typedef struct {
  uint8_t reliable; //!< Whether reliable writes requested (always FALSE for
                    //!< Write Long)
 ...
 */
typedef struct {
  uint8_t reliable; //!< Whether reliable writes requested (always TRUE for
                    //!< Reliable Writes)
  attPrepareWriteReq_t
...
typedef struct attAttribute_t {
  ...
  uint8_t *pValue; //!< Attribute value - encoding of the octet array is defined
                   //!< in the applicable profile. The maximum length of an
                   //!< attribute value shall be 512 octets.
} gattAttribute_t;
...
typedef struct {
  ...
  uint8_t encKeySize; //!< Minimum encryption key size required by service (7-16
                      //!< bytes)
 ...
 *
 * @note    If blePending is returned then it's the responsibility of the
 * application to respond to ATT_READ_REQ and ATT_READ_BLOB_REQ message with
 * ATT_READ_RSP and ATT_READ_BLOB_RSP message respectively.
 *
 * @note    Payload 'pValue' used with ATT_READ_RSP and ATT_READ_BLOB_RSP must
 * be allocated using GATT_bm_alloc().
 *
 ...
 *          - Reliable Writes: Multiple ATT_PREPARE_WRITE_REQ followed by one
 * final ATT_EXECUTE_WRITE_REQ
 *
 * @note    If blePending is returned then it's the responsibility of the
 * application to 1) respond to ATT_WRITE_REQ and ATT_EXECUTE_WRITE_REQ message
 * with ATT_WRITE_RSP and ATT_EXECUTE_WRITE_RSP message respectively, and 2)
 * free each request payload 'pValue' using BM_free().
 *
...
/**
 * GATT Structure for service callback functions - must be setup by the
 * application and used when GATTServApp_RegisterService() is called.
 */
typedef struct {
  ...
  pfnGATTAuthorizeAttrCB_t
      pfnAuthorizeAttrCB; //!< Authorization callback function pointer
} gattServiceCBs_t;
 ...
 * application in case it wants to save it to be restored on reboot or
 * reconnect. This message is sent to update a connection's signature counter
 * and to update this device's signature counter.  If devAddr == BD_ADDR, then
 * this message pertains to this device.
 */
typedef struct {
  tmos_event_hdr_t hdr;        //!< GAP_MSG_EVENT and status
...
typedef struct {
  ...
  uint8_t eventType;        //!< Advertisement Type: @ref
...
typedef struct {
  ...
  uint8_t advertisingSID; //!< Value of the Advertising SID subfield in the ADI
                          //!< field of the PDU
...
typedef struct {
  ...
  uint8_t whiteList;  //!< TRUE to only allow advertisements from devices in the
                      //!< white list.
} gapDevDiscReq_t;
 ...
 */
typedef struct {
  uint8_t eventType; //!< Indicates advertising event type used by the
                     //!< advertiser: @ref GAP_ADVERTISEMENT_REPORT_TYPE_DEFINES
  uint8_t addrType;  //!< Address Type: @ref GAP_ADDR_TYPE_DEFINES
...
/**
 * GAP_PERIODIC_ADVERTISING_DONE_EVENT message format.  This message is sent to
 * the app when the Periodic Advertising config is complete.
 */
typedef struct {
  tmos_event_hdr_t hdr; //!< GAP_MSG_EVENT and status
...
/**
 * GAP_PERIODIC_ADV_DEVICE_INFO_EVENT message format.  This message is sent to
 * the app during Periodic Advertising Sync, when received a Periodic
 * Advertising packet
 */
typedef struct {
  tmos_event_hdr_t hdr; //!< GAP_MSG_EVENT and status
 ...
 * <BR>
 * For an Observer, this message is sent to complete the Establish Link
 * Request.<BR> For a Peripheral, this message is sent to indicate that a link
 * has been created.
 */
typedef struct {
  tmos_event_hdr_t hdr; //!< GAP_MSG_EVENT and status
  ...
  uint16_t connectionHandle; //!< Connection Handle from controller used to ref
                             //!< the device
...
/**
 * GAP_PHY_UPDATE_EVENT message format.  This message is sent to the
 * app(GAP_MSG_EVENT) when the PHY update request is complete.
 */
typedef struct {
  tmos_event_hdr_t hdr;      //!< GAP_MSG_EVENT and status
...
typedef struct {
  ...
  uint8_t deviceAddr[B_ADDR_LEN]; //!< address of device to pair with, and could
                                  //!< be either public or random.
  uint16_t connectionHandle;      //!< Connection handle
  uint8_t
      uiInputs; //!< Pairing User Interface Inputs - Ask user to input passcode
...
typedef void (*pfnPasscodeCB_t)(
    uint8_t *deviceAddr,       //!< address of device to pair with, and could be
                               //!< either public or random.
    uint16_t connectionHandle, //!< Connection handle
    uint8_t uiInputs, //!< Pairing User Interface Inputs - Ask user to input
                      //!< passcode
    uint8_t uiOutputs //!< Pairing User Interface Outputs - Display passcode
);
...
#define GAPROLE_WAITING                                                        \
  3 //!< Device is started but not advertising, is in waiting period before
    //!< advertising again
...
// Periodic advertising Enable,only effective when
// GAP_ADTYPE_EXT_NONCONN_NONSCAN_UNDIRECT advertising event enable
...
#define GAPROLE_PERIODIC_INVALID                                               \
  (0 << 4) //!< Periodic advertising Waiting to be started
#define GAPROLE_PERIODIC_ENABLE (1 << 4) //!< Periodic advertising Enable
#define GAPROLE_PERIODIC_WAIT                                                  \
  (2 << 4) //!< Periodic advertising is started but disable
...
#define GAPROLE_PERIODIC_STATE_VALID                                           \
  (1 << 24) //!< indicates periodic advertising states change
...
typedef union {
  ...
  gapDeviceInfoEvent_t
      deviceInfo; //!< Discovery device information event structure.
  gapDirectDeviceInfoEvent_t deviceDirectInfo; //!< Discovery direct device
                                               //!< information event structure.
  gapAdvDataUpdateEvent_t dataUpdate; //!< Advertising Data Update is complete.
  gapPeriodicAdvDeviceInfoEvent_t
      devicePeriodicInfo; //!< Discovery periodic device information event
                          //!< structure.
  gapExtAdvDeviceInfoEvent_t
      deviceExtAdvInfo; //!< Discovery extend advertising device information
                        //!< event structure.
  ...
  gapScanReqReseiveEvent_t
      scanReqEvt; //!< Scan_Request_Received event structure.
...
typedef struct {
  uint8_t eventType; //!< Indicates advertising event type used by the
                     //!< advertiser: @ref GAP_ADVERTISEMENT_REPORT_TYPE_DEFINES
  uint8_t addrType;  //!< Scan Address Type:0x00-Public Device Address or Public
                     //!< Identity Address 0x01-Random Device Address or Random
...
typedef struct {
  ...
  /*
   bit0: used to determine whether the Periodic  Advertiser List is used
     0: Use the Advertising_SID, Advertisier_Address_Type, and
   Advertiser_Address parameters to determine which advertiser to listen to. 1:
   Use the Periodic Advertiser List to determine which advertiser to listen to.
   bit1: whether GAP_PERIODIC_ADV_DEVICE_INFO_EVENT events for this periodic
   advertising train are initially enabled or disabled. 0: Reporting initially
   enabled 1: Reporting initially disabled bit2: 0: Duplicate filtering
   initially disabled 1: Duplicate filtering initially enabled */
  uint8_t advertising_SID; //!< if used, specifies the value that must match the
                           //!< Advertising SID
  uint8_t addrType;        //!< Scan Address Type: @ref GAP_ADDR_TYPE_DEFINES
  uint8_t addr[B_ADDR_LEN]; //!< Device's Address
  uint16_t
      skip; //!< the maximum number of consecutive periodic advertising events
            //!< that the receiver may skip after successfully receiving a
            //!< periodic advertising packet.Range: 0x0000 to 0x01F3
  uint16_t syncTimeout; //!< the maximum permitted time between successful
                        //!< receives. If this time is exceeded, synchronization
                        //!< is lost. Time = N*10 ms.Range: 0x000A to 0x4000
  uint8_t
      syncCTEType; //!< specifies whether to only synchronize to periodic
                   //!< advertising with certain types of Constant Tone
                   //!< Extension (a value of 0 indicates that the presence or
...
typedef struct {
  ...
  uint16_t minTimeSpent; //!< Minimum time in number of connection events to be
                         //!< observed once the path crosses the threshold
...
typedef struct {
  ...
  int8_t
      highRxThreshold; //!< High threshold for the peer power levels.Units: dB
...
typedef struct {
  gapRolesBroadcasterStateNotify_t
      pfnStateChange; //!< Whenever the device changes state
 ...
 * gapRoles_StartDevice() is called.
 */
typedef struct {
  ...
  gapRolesParamUpdateCB_t
      pfnParamUpdate; //!< When the connection parameteres are updated
} gapRolesCBs_t;
...
#define TX_MODE_TX_FAIL                                                        \
  0x11 //!< basic or auto tx mode fail to send data and enter idle state
#define TX_MODE_TX_TIMEOUT TX_MODE_TX_FAIL //!< time of data transmission
#define TX_MODE_RX_DATA                                                        \
  0x02 //!< auto tx mode receive data(ack) and enter idle state
#define TX_MODE_RX_TIMEOUT                                                     \
  0x12 //!< auto tx mode receive timeout and enter idle state
...
#define RX_MODE_RX_DATA                                                        \
  0x03 //!< basic or auto rx mode receive data
       ...
       //!< if it matches,it will send data(ack),otherwise(rsr=2), it will
       //!< restart receiving
#define RX_MODE_TX_FINISH                                                      \
  0x04 //!< auto rx mode sends data(ack) successfully and enters idle state
#define RX_MODE_TX_FAIL                                                        \
  0x14 //!< auto rx mode fail to send data and enter idle state
#define RX_MODE_TX_TIMEOUT RX_MODE_TX_FAIL //!< time of data transmission
#define RX_MODE_HOP_SHUT 0x24

// LLE_MODE_TYPE
#define LLE_MODE_BASIC                                                         \
  (0) //!< basic mode, enter idle state after sending or receive
#define LLE_MODE_AUTO                                                          \
  (1) //!< auto mode, auto swtich to the receiving status after sending and the
      //!< sending status after receiving
...
typedef struct tag_rf_config {
  uint8_t
      LLEMode; //!< BIT0   0=basic, 1=auto def@LLE_MODE_TYPE
               //!< BIT1   0=whitening on, 1=whitening off
               //!< def@LLE_WHITENING_TYPE BIT4-5 00-1M  01-2M  10-coded(S8)
               //!< 11-coded(S2) def@LLE_PHY_TYPE BIT6   0=data channel(0-39)
               //!<        1=rf frequency (2400000kHz-2483500kHz)
               //!< BIT7   0=the first byte of the receive buffer is rssi
               //!<        1=the first byte of the receive buffer is package
               //!<        type
  ...
  uint32_t
      ChannelMap; //!< indicating  Used and Unused data channels.Every channel
                  //!< is represented with a bit positioned as per the data
                  //!< channel index,The LSB represents data channel index 0
  uint8_t Resv;
  uint8_t HeartPeriod; //!< The heart package interval shall be an integer
                       //!< multiple of 100ms
  uint8_t HopPeriod;   //!< hop period( T=32n*RTC clock ),default is 8
  uint8_t HopIndex;    //!< indicate the hopIncrement used in the data channel
                       //!< selection algorithm,default is 17
...
/**
 * @brief   clear a event already timeout, cannot be used in it own event
 * function.
 *
 * @param   taskID - task ID of event
 * @param   event - event value
 ...
 * @return  TRUE,FALSE.
 */
extern BOOL tmos_start_task(tmosTaskID taskID, tmosEvents event,
                            tmosTimer time);

/**
 * @brief   This function is called to start a timer to expire in n system clock
 * time. When the timer expires, the calling task will get the specified event
...
/**
 * @brief   Register a callback function will be called after each connect
 * event. Only effect in single connection
 *
...
/**
 * @brief   Register a callback function will be called after each advertise
 * event.
 *
 ...
 * It shall not be a sequence that differ s from the advertising channel
 * packets' Access Address by only one bit. It shall not have all four octets
 * equal. It shall have no more  than 24 transitions. It shall have a minimum of
 * two transitions in the most significant six bits.
 */
...
/*
 * linkDB_PerformFunc - Perform a function of each connection in the link
 * database.
 */
 ...
 */
#define linkDB_Up(connectionHandle)                                            \
  linkDB_State((connectionHandle), LINK_CONNECTED)
 ...
 *          (with SUCCESS or bleTimeoutstatus) is received by the calling
 * application task.
 *
 * @param   connHandle - connection to use
 ...
 *
 * @note    A notification may be sent at any time and does not invoke a
 * confirmation. No confirmation will be sent to the calling application task
 ...
 *          or ATT_ERROR_RSP (if an error occurred on the server).
 *
 * @note    This sub-procedure is complete when either
 * ATT_FIND_BY_TYPE_VALUE_RSP (with bleProcedureComplete or bleTimeout status)
 * or ATT_ERROR_RSP (with SUCCESS status) is received by the calling application
 * task.
 *
 * @param   connHandle - connection to use
 ...
 *          (with SUCCESS status) is received by the calling application task.
 *
 * @note    The 'pReq->pValue' pointer will be freed when the sub-procedure is
 * complete.
 *
 * @param   connHandle - connection to use
 * @param   pReq - pointer to request to be sent
 ...
 *
 * @note    This function should only be called by GATT and the upper layer
 ...
 * @param   encKeySize - Minimum encryption key size required by service (7-16
 * bytes)
 ...
 *
 * @note    Each client has its own instantiation of the ClientCharacteristic
 * Configuration. Reads/Writes of the Client Characteristic Configuration only
 * only affect the configuration of that client.
 *
 ...
 *
 * @note    Each client has its own instantiation of the Client Characteristic
 * Configuration. Reads of the Client Characteristic Configuration only shows
 * the configuration for that client.
 *
 * @param   connHandle - connection handle.
 * @param   charCfgTbl - client characteristic configuration table.
 *
 ...
 *
 * @note    Each client has its own instantiation of the Client Characteristic
 * Configuration. Writes of the Client Characteristic Configuration only only
...
/**
 * @brief   Set a GAP Parameter value.  Use this function to change  the default
...
/**
 * @brief   Setup the device's address type.  If ADDRTYPE_PRIVATE_RESOLVE is
 * selected, the address will change periodically.
 *
 * @param   addrType - @ref GAP_ADDR_TYPE_DEFINES
 * @param   pStaticAddr - Only used with ADDRTYPE_STATIC or
 * ADDRTYPE_PRIVATE_NONRESOLVE type NULL to auto generate otherwise the
 * application can specify the address value
 *
 ...
 *          bleIncorrectMode: can't change with an active connection,or
 * INVALIDPARAMETER If return value isn't SUCCESS, the address type remains the
 * same as before this call.
 */
 ...
 *
 * @note    if the return status from this function is SUCCESS,the task isn't
 * complete until the GAP_ADV_DATA_UPDATE_DONE_EVENT is sent to the calling
 * application task.
 *
 * @param   taskID - task ID of the app requesting the change
 * @param   adType - TRUE - advertisement data, FALSE  - scan response data
 * @param   dataLen - Octet length of advertisement_data
 ...
 *
 * @note    You can call this function with a GAP Parameter ID and it will set
 * the GAP Parameter.
 *
 ...
 *
 * @param   connectionHandle - connection handle of the connected device or
 * 0xFFFF if all devices in database.
 * @param   status - SUCCESS if passcode is available, otherwise see @ref
 * SMP_PAIRING_FAILED_DEFINES.
 ...
 * parameter ID and WILL be cast to the appropriate data type (example: data
...
/**
 * @brief   used to synchronize with a periodic advertising train from an
...
/**
 * @brief   used to cancel the HCI_LE_Periodic_Advertising_Create_Sync command
 * while it is pending.
 *
 * @param   None.
 *
 ...
 *
 * @param   connHandle - connection handle
 * @param   all_phys - a bit field that  allows the Host to specify, for each
 * direction set BIT0:The Host has no preference among the transmitter PHYs
 * supported by the Controller set BIT1:The Host has no preference among the
 * receiver PHYs supported by the Controller
 * @param   tx_phys - a bit field that indicates the transmitter
 * PHYs.(GAP_PHY_BIT_TYPE)
 * @param   rx_phys - a bit field that indicates the receiver
 * PHYs.(GAP_PHY_BIT_TYPE)
 * @param   phy_options - preferred coding when transmitting on the LE Coded
 * PHY(GAP_PHY_OPTIONS_TYPE)
 *
 * @return  SUCCESS: PHY update started started .<BR>
 *          bleIncorrectMode: No connection to update.<BR>
 */
extern bStatus_t GAPRole_UpdatePHY(uint16_t connHandle, uint8_t all_phys,
                                   uint8_t tx_phys, uint8_t rx_phys,
                                   uint16_t phy_options);

/**
 * @brief   used to allow the Host to specify the privacy mode to be used  for a
 * given entry on the resolving list.
 *
 * @note    This command shall not be used when address resolution is enabled in
 * the Controller and: Advertising (other than periodic advertising) is enabled,
 *          Scanning is enabled, or
 *          an GAPRole_CentralEstablishLink, or GAPRole_CreateSync command is
 * pending.
 *
 * @param   addrTypePeer - 0x00:Public Identity Address 0x01:Random (static)
 * Identity Address
 * @param   peerAddr - Public Identity Address or Random (static) Identity
 * Address of the advertiser
 * @param   privacyMode - 0x00:Use Network Privacy Mode for this peer device
 ...
 */
extern bStatus_t
GAPRole_SetPathLossReporting(gapRoleSetPathLossReporting_t *pParm);
 ...
 *
 * @param   the ID assigned by tmos.This ID should be used to send messages and
 * set timers.
 *
 ...
 */
extern bStatus_t
GAPRole_BroadcasterStartDevice(gapRolesBroadcasterCBs_t *pAppCallbacks);
 ...
 */
extern bStatus_t
GAPRole_ObserverStartDevice(gapRoleObserverCB_t *pAppCallbacks);
 ...
 */
extern bStatus_t GAPRole_PeripheralConnParamUpdateReq(
    uint16_t connHandle, uint16_t connIntervalMin, uint16_t connIntervalMax,
 ...
 * @param   addrTypePeer - address type of the peer device: @ref
 ...
 * @brief
 *
 * @param   timeoutMS - Maximum time to wait for receiving HOP_TX pdu(Time = n *
 * 1mSec),0 = unlimited.
 *
...
/**
 * @brief   used to start a test where the DUT receives test reference packets
 * at a fixed interval
 *
 * input parameters
 *
...
/**
 * @brief   used to start a test where the DUT generates test reference packets
 * at a fixed interval
 *
 * @param   opcode = 0x201E
 *              pParm 0 - TX_Channel
 ...
 *
 * @param   pParm(in) - Must provide length of parameter followed by 6 bytes
 * parameter
 *
 * @return  Command Status.
 */
